...
include 'dbc.php';
$pageTitle = "Add a new Product Category and other attributes";
$pageName    = "Product Category";
$linkName    = "Show All Product Category";
$editMode    =   "editData";
$addMode    =   "addData";
$tableName   = $tblProductCategory;
$listPageUrl = $urlProductCatgoryList;
$imageUrl = $mainPath;
                            ...
                            <form id="InsertUpdateForm" role="form" action="" method="post"
                                ...
                                <div class="form-group row">
                                    <div class="col-xs-12 col-sm-12">
                                        ...
                                        <select name="parent_id" class="form-control js-source-states">
                                            <option value="">Select Parent Category</option>
                                            <?php
                                            // Query to fetch all categories (parent and child)
                                            $category_que = "SELECT * FROM tbl_product_category ORDER BY parent_id, name";
                                            $category_res = mysqli_query($conn, $category_que);

                                            // Create an array to store categories by parent_id
                                            $categories = [];
                                            while ($category_row = mysqli_fetch_array($category_res)) {
                                                $categories[$category_row['parent_id']][] = $category_row;
                                            }

                                            // Display parent categories first (parent_id = 0)
                                            if (isset($categories[0])) {
                                                foreach ($categories[0] as $parent_category) {
                                                    $selected = isset($serviceData['parent_id']) && $serviceData['parent_id'] == $parent_category['id'] ? 'selected' : '';
                                                    echo "<option value='{$parent_category['id']}' {$selected}>{$parent_category['name']}</option>";

                                                    // Check if the parent has child categories
                                                    if (isset($categories[$parent_category['id']])) {
                                                        echo "<optgroup label='-- Subcategories of {$parent_category['name']} --'>";

                                                        // Display child categories
                                                        foreach ($categories[$parent_category['id']] as $child_category) {
                                                            $selected = isset($serviceData['parent_id']) && $serviceData['parent_id'] == $child_category['id'] ? 'selected' : '';
